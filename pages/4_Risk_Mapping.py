...
st.set_page_config(page_title="Risk Mapping", layout="wide")

def main():
    st.title("Fire Risk Mapping & Monitoring")
...
def show_risk_assessment_map(risk_levels, region_center):
    ...
    with col2:
        ...
        if len(high_zones) > 0:
            st.warning(f"{len(high_zones)} HIGH risk zones")
...
def show_real_time_monitoring(time_range, region_center):
    ...
    tab1, tab2, tab3 = st.tabs(["Satellite Detection", "Weather Monitoring", "Alert Dashboard"])
    ...
    with tab1:
        ...
        with col2:
            ...
            for _, det in recent.iterrows():
                st.markdown(f"{det['detection_time']} - {det['confidence']:.1%}")
    
    with tab2:
        st.subheader("Weather Station Network")
        ...
        with col2:
            ...
            if len(high_temp_stations) > 0:
                st.error(f"{len(high_temp_stations)} stations reporting high temps")
...
def show_weather_overlay_map(region_center):
    ...
    st.header("Weather Conditions Overlay")
